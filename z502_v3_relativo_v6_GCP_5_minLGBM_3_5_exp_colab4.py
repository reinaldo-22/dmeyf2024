...
joblib.dump( [ original_columns,original_columns_inta_mes,  data,  features_finales, feature_importance_df_ranking, feature_importance_df_bool ], exp_folder+ 'aacc1.joblib')
data_x.write_parquet(desktop_folder+'mini_data_x.parquet' )
...
def objective(trial):   
    ...
    if  trial_number >51:
        params['learning_rate'] = best_predictions.suggest_float("learning_rate", 0.3, 0.7)   
        params['feature_fraction'] = best_predictions.suggest_float("feature_fraction", 0.3, 0.9)
        params['num_leaves'] = best_predictions.suggest_int("num_leaves", 8, 2048)
        params['num_iterations'] = best_predictions.suggest_int("num_iterations", 5, 50)
        min_data_in_leaf = best_predictions.suggest_float("min_data_in_leaf",  1.5E-05, 0.009)  # Example of leaf size    
        params['bagging_fraction'] = best_predictions.suggest_float("bagging_fraction", 0.1, 1)
        
    params['neg_bagging_fraction'] = 0.05# trial.suggest_float("neg_bagging_fraction", 0.02, 0.35)  # Example of leaf size    
...
#    fraction = 0.1# trial.suggest_float('fraction', 0.01, 1)             
    ...
    if  trial_number>51:
        ...
        for col_origianl in en_duda:
            ...
            if trial.suggest_categorical(str(col_origianl), [True, False]):
                print(col_origianl)
    ...
    if trial_number ==51 or trial_number == 251:
        final_res = []
        for i in range (0,49):
            random_state +=i
