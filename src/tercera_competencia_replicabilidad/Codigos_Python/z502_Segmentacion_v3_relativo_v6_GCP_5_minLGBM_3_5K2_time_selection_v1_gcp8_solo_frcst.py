...
def create_data(ganancia_acierto,  path_set_crudo, path_set_con_ternaria,  mes_train, mes_test ):
    ...
    data = pd.read_csv(path_set_con_ternaria)    
    ...
    data = add_forecast(data)
    data.to_csv(desktop_folder + 'competencia_03_forecast.csv')    
...
source = desktop_folder+'data_x_second_boruta.parquet'
destination = "/home/reinaldomedina_robledo/buckets/b2/comp_3_exp/escopeta_3_1/data_x_final.parquet"
shutil.copy(source, destination)  # Use shutil.copy2 for preserving metadata


project_id = "reflected-wind-441920-p5"
zone = "southamerica-east1-c"
instance_name = "temp-08vcpu-256ram-20241127-140952"
...
def list_projects():
    """Fetches all GCP projects you have access to."""
    client = resource_manager.Client()
    for project in client.list_projects():
        print(f"Project ID: {project.project_id}, Name: {project.name}, Status: {project.status}")
...
try:
    ...
    print("The source file or destination does not exist.")
...
'mpayroll', 'mpayroll2', 'cpayroll2_trx', 'ccuenta_debitos_automaticos', 'mcuenta_debitos_automaticos', 'ctarjeta_visa_debitos_automaticos', 'mttarjeta_visa_debitos_automaticos', 'ctarjeta_master_debitos_automaticos', 'mttarjeta_master_debitos_automaticos', 'cpagodeservicios', 'mpagodeservicios', 'cpagomiscuentas', 'mpagomiscuentas' ,'mcomisiones_otras' , 'chomebanking_transacciones', 'ctrx_quarter' ,
 ...
 'Visa_msaldototal', 'Visa_msaldopesos', 'Visa_msaldodolares', 'Visa_mconsumospesos', 'Visa_mconsumosdolares', 'Visa_mlimitecompra', 'Visa_madelantopesos']
    """
    cols = ['foto_mes', 'active_quarter', 'mrentabilidad', 'mrentabilidad_annual', 'mcomisiones', 'mactivos_margen', 'mpasivos_margen', 'cproductos', 'tcuentas', 'ccuenta_corriente', 'ccaja_ahorro', 'mcaja_ahorro', 'cdescubierto_preacordado', 'mcuentas_saldo', 'ctarjeta_debito', 'ctarjeta_visa', 'ctarjeta_visa_transacciones', 'mtarjeta_visa_consumo', 'ctarjeta_master', 'ccomisiones_otras', 'mcomisiones_otras', 'chomebanking_transacciones', 'ctrx_quarter', 'cmobile_app_trx', 'Master_mfinanciacion_limite', 'Master_Fvencimiento', 'Master_Finiciomora', 'Master_mconsumospesos', 'Master_mlimitecompra', 'Master_fultimo_cierre', 'Master_mpagospesos', 'Master_fechaalta', 'Master_mconsumototal', 'Master_cconsumos', 'Visa_mfinanciacion_limite', 'Visa_Fvencimiento', 'Visa_Finiciomora', 'Visa_msaldototal', 'Visa_msaldopesos', 'Visa_mconsumospesos', 'Visa_mlimitecompra', 'Visa_fultimo_cierre', 'Visa_mpagospesos', 'Visa_fechaalta', 'Visa_mconsumototal', 'Visa_cconsumos', 'Visa_mpagominimo']
    numeric_features= list( set( cols)- set( ['foto_mes', 'numero_de_cliente', 'clase_ternaria'] ) )
    ...
    res_f = pd.DataFrame( columns= [ col+'_forcst' for col in numeric_features ], index =group_df.index[1:] )
    #res_f = pl.from_pandas( res_f)
    return res_f
...
base = pd.read_csv('/home/reinaldo_medina_gcp/buckets/b2/expw227_c3_v3/KA-0001/KA-0001_01_009_r1_09800.csv')
experimento= '/home/reinaldo_medina_gcp/buckets/b2/expw227_c3_v3_FRCST_2/KA-0001/'

a_enmendar = pd.read_csv (experimento + 'KA-0001_01_029_r1_09900.csv')

result = pd.merge(a_enmendar, base, on='numero_de_cliente', how='outer', suffixes=('_a', '_b'))
result['Predicted'] = result['Predicted_a'].combine_first(result['Predicted_b'])
result = result[['numero_de_cliente', 'Predicted']].reset_index(drop=True).astype(int)
result.to_csv( experimento+ 'enmienda2KA-0001_01_029_r1_09900.csv', index=False)
np.sum(result['Predicted'])
...
experimento_f1 = pd.read_csv (experimento_f1 + 'KA-0001_01_028_r1_10400.csv')
...
merged = pd.merge(merged, expperimento_b3, on='numero_de_cliente', how='outer')
...
expperimento_b3 = pd.read_csv (expperimento_b3 + 'KA-0001_01_009_r1_09800.csv')
experimento_f1 = pd.read_csv (experimento_f1 + 'KA-0001_01_028_r1_09900.csv')
experimento_f2 = pd.read_csv (experimento_f2 + 'KA-0001_01_029_r1_09900.csv')
...
merged.to_csv( experimento+ 'frcts1_9900_frcts2_9900_b2_10250-ok.csv', index=False)

old= pd.read_csv( experimento+ 'frcts1_9900_frcts2_9900_b2_10250.csv')
np.sum(old['Predicted'])
assert merged['Predicted'].equals(old['Predicted'])
Out[73]: np.int64(10862)
...
expperimento_b2 = pd.read_csv (expperimento_b2 + 'KA-0002_01_019_r1_10250.csv')
expperimento_b3 = pd.read_csv (expperimento_b3 + 'KA-0001_01_009_r1_09800.csv')
experimento_f1 = pd.read_csv (experimento_f1 + 'KA-0001_01_028_r1_10100.csv')
experimento_f2 = pd.read_csv (experimento_f2 + 'KA-0001_01_029_r1_10100.csv')
...
np.sum(merged['Predicted'])
merged= merged[['numero_de_cliente','Predicted']]
merged.to_csv( experimento+ 'frcts1y2_10100_b2_10250.csv', index=False)
np.int64(10978)

################
    
base = pd.read_csv('/home/reinaldo_medina_gcp/buckets/b2/expw227_c3_v3/KA-0001/KA-0001_01_009_r1_09800.csv')
experimento= '/home/reinaldo_medina_gcp/buckets/b2/expw227_c3_v3_FRCST/KA-0001/'

a_enmendar = pd.read_csv (experimento + 'KA-0001_01_028_r1_10000.csv')

result = pd.merge(a_enmendar, base, on='numero_de_cliente', how='outer', suffixes=('_a', '_b'))
result['Predicted'] = result['Predicted_a'].combine_first(result['Predicted_b'])
result = result[['numero_de_cliente', 'Predicted']].reset_index(drop=True).astype(int)
result.to_csv( experimento+ 'efrct1-0001_01_029_r1_09900.csv', index=False)
np.sum(result['Predicted'])

merged = pd.merge(experimento_f1, expperimento_b3, on='numero_de_cliente', suffixes=('_f1', '_f2'))

# Find mismatches in Predicted columns
mismatches = (merged['Predicted_f1'] != merged['Predicted_f2']).sum()
