...
def time_features_mini(df_train: pl.DataFrame) -> pl.DataFrame:
    # Sort dataframe
...
def create_data(ganancia_acierto, path_set_crudo, path_set_con_ternaria,  mes_train, mes_test ):
    ...
    data = data.filter(pl.col('foto_mes') >= 202011)
    data = data.filter(pl.col('foto_mes') <= 202106)
    ...
    data= convert_to_int_float32_polars(data)
    lag_flag, delta_lag_flag = True, False
...
def create_data_sin_boruta(ganancia_acierto, path_set_crudo, path_set_con_ternaria,  mes_train, mes_test ):
    if not os.path.exists(path_set_con_ternaria):
        crete_ternaria( path_set_crudo, path_set_con_ternaria)     
    ...
    return original_columns ,original_columns_inta_mes,  data
...
mes_train, mes_test = 202102, 202104 # para seleccion features
nombre_exp_study = 'study_MiniLGBM.joblib'

exp_folder = '/home/reinaldomedina_robledo/buckets/b2/exp/colaborativo2_sin_boruta/'
exp_folder = '/home/reinaldomedina_robledo/buckets/b2/exp/colaborativo2/'
path_set_crudo= None
path_set_con_ternaria = '/home/reinaldomedina_robledo/Desktop/competencia_02_ternaria.parquet'
desktop_folder= '/home/reinaldomedina_robledo/Desktop/'

original_columns,original_columns_inta_mes,  data,  features_finales, feature_importance_df_ranking, feature_importance_df_bool = create_data(ganancia_acierto, path_set_crudo, path_set_con_ternaria,  mes_train, mes_test )


"""
# grabado

joblib.dump( [ original_columns,original_columns_inta_mes,    features_finales, feature_importance_df_ranking, feature_importance_df_bool ], exp_folder+ 'aacc1.joblib')
data.write_parquet(desktop_folder+'mini_data_x.parquet' )

"""
"""
#Lectura
data_x = pl.read_parquet(desktop_folder+'mini_data_x.parquet' )
[ original_columns,original_columns_inta_mes, features_finales, feature_importance_df_ranking, feature_importance_df_bool]= joblib.load(exp_folder+ 'aacc1.joblib')

"""
"""
sin boruta

original_columns ,original_columns_inta_mes,  data = create_data_sin_boruta(ganancia_acierto, path_set_crudo, path_set_con_ternaria,  mes_train, mes_test )
...
params = { 
    ...
    "max_bin": 90,  # lo debo dejar fijo, no participa de la BO
...
def create_LGBM_dataset(final_selection, trains, mes_test, data_x, clase_peso_lgbm,params):
    ...
    if type(final_selection) == list:
        final_selection= list( set(final_selection))+['clase_ternaria','foto_mes', 'clase_peso']
        data_x_selected = data_x.select(final_selection)
        df_test = data_x.select(final_selection)
        #data_x_selected= data_x[final_selection]
        #df_test = data_x[final_selection]
    else:
    ...
    resample = data_x_selected.filter(    
    (pl.col("clase_ternaria") == 'CONTINUA') )
    
    #pesos = np.exp(resample['res_mean']+0.01)
    chosen_rows = resample.sample(
    n=int(len(resample) * params['neg_bagging_fraction']), 
    ...
    w_train = df_train_3['clase_peso']       
    X_train = df_train_3.drop(['clase_ternaria',  'clase_peso'])
    ...
    df_test = df_test.filter(pl.col('foto_mes') == mes_test)
    ...
    w_test = df_test['clase_peso']
    X_test = df_test.drop(['clase_ternaria',  'clase_peso'])
...
def objective(trial):   
    study = trial.study
    best_predictions = study.user_attrs.get('best_predictions')   
    ...
    if  trial_number <=150 :
        params['learning_rate'] = trial.suggest_float("learning_rate", 0.3, 0.7)   
        ...
        params['num_iterations'] = trial.suggest_int("num_iterations", 5, 50)
        min_data_in_leaf = trial.suggest_float("min_data_in_leaf",  1.5E-05, 0.009)  # Example of leaf size    
        params['bagging_fraction'] = trial.suggest_float("bagging_fraction", 0.1, 1)
    if  trial_number >150:
        params['learning_rate'] = best_predictions_base.suggest_float("learning_rate", 0.3, 0.7)   
        params['feature_fraction'] = best_predictions_base.suggest_float("feature_fraction", 0.3, 0.9)
        params['num_leaves'] = best_predictions_base.suggest_int("num_leaves", 8, 2048)
        params['num_iterations'] = best_predictions_base.suggest_int("num_iterations", 5, 50)
        min_data_in_leaf = best_predictions_base.suggest_float("min_data_in_leaf",  1.5E-05, 0.009)  # Example of leaf size    
        params['bagging_fraction'] = best_predictions_base.suggest_float("bagging_fraction", 0.1, 1)
        
    params['neg_bagging_fraction'] = 0.02# trial.suggest_float("neg_bagging_fraction", 0.02, 0.35)  # Example of leaf size    
...
#    fraction = 0.1# trial.suggest_float('fraction', 0.01, 1)             
    ...
    if  trial_number>150:
        intocables= []
        for col_origianl in remaning_originals: 
            for col_final in top_15_feature_names:
                if col_origianl in col_final:
                    intocables.append( col_origianl)
                    print( 'intocables', intocables)
            
        en_duda= list( set(remaning_originals) -set(intocables) )
        print('en duda', en_duda)
        origianal_selection=[]    
        #if  trial_number >=5:
        for col_origianl in en_duda:
            
            if best_predictions.suggest_categorical(str(col_origianl), [True, False]):
                print(col_origianl)
                origianal_selection.append(col_origianl)
        print('origianal_selection', origianal_selection)
        final_selection=[]                
        for col_origianl in origianal_selection:         
           for  col_final in features_finales:
               if col_origianl in col_final:
                   final_selection.append(col_final )
                   
        print('final_selection' , final_selection)
        
        print( 'dif', list( set( features_finales)- set(final_selection)  ) )
        print(len(final_selection ))
        print(len(origianal_selection ))
        print(len(en_duda ))
    else:
    ...
    trains = [202101, 202102, 202103, 202104]
    ...
    if trial_number==151 or trial_number == 600:
        final_res = []
        for i in range (0,49):
            random_state +=i
            ...
            for rnd in random_numbers:
                ...
                print('dataset time', time.time() -wstart)
                ...
                times.append(  time.time() - mini_start )
            ...
            final_res.append(res0)
        joblib.dump( final_res, exp_folder+ str(trial_number)+'_50.joblib')
    ...
    for rnd in random_numbers:
        ...
        if welapsed_time >1:
            ...
            return res0 , min( times) * cant_semillas_ensamble #- len(feature_selection )*penalty, time
...
top_15_feature_names= feature_importance_df_ranking['feature'][:7]

remaning_originals = list(set( original_columns) .intersection( set(features_finales) ) )
...
for i in range(54, 600):
    #study.optimize(objective, n_trials=1)  # You can specify the number of trials
    #best_predictions= None
    trials_df = study.trials_dataframe()
    if trials_df['number'].max()==150:
        max_row = trials_df.iloc[152:,:]['values_0'].idxmax()
        max_number = max_row['number']
        best_predictions= study.trials[max_row]
        best_predictions_base= study.trials[5]
        study.set_user_attr('best_predictions', best_predictions)   
...
nombre_exp_study= 'stage_2.joblib'
...
trials_df = study.trials_dataframe()
max_row = trials_df.loc[trials_df['values_0'].idxmax()]
max_number = max_row['number']
best_predictions= study.trials[max_number]
study.set_user_attr('best_predictions', best_predictions)
...
# Extract trials 0 to 50
trials_subset = trials_df.iloc[:151]

# Sort the 'value_0' column
sorted_values = trials_subset['values_0'].sort_values()

# Plot the sorted values
plt.figure(figsize=(8, 6))
plt.plot(sorted_values.values, marker='o', linestyle='-', color='b')
plt.title('Trials 0-151')
plt.xlabel('Trial Index')
plt.ylabel('Sorted Value_0')
plt.grid(True)
plt.show()


"""
intocables ['mautoservicio', 'minversion1_pesos', 'mplazo_fijo_dolares', 'ctarjeta_debito_transacciones']
top_15_feature_names
Out[7]: 
729        vmr_msaldopesos_rank_lag1
728        vmr_msaldototal_rank_lag1
723           vm_cconsumos_rank_lag1
22     ctarjeta_debito_transacciones
23                     mautoservicio
37               mplazo_fijo_dolares
40                 minversion1_pesos
"""

ref= joblib.load(exp_folder+ '151_50.joblib')
exp_a= joblib.load(exp_folder+ '600_50.joblib')
import numpy as np
import matplotlib.pyplot as plt
...
t_test_result = ttest_rel(exp_a, ref)

# Wilcoxon signed-rank test
wilcoxon_result = wilcoxon(exp_a, ref)

# Display results
print("Paired t-test:")
print(f"Statistic: {t_test_result.statistic}, p-value: {t_test_result.pvalue}")
...
bins = np.linspace(min(min(exp_a), min(ref)), max(max(exp_a), max(ref)), 10)



plt.figure(figsize=(10, 6))
plt.hist(exp_a, bins, alpha=0.6, label='exp_a', color='blue', edgecolor='black')
plt.hist(ref, bins, alpha=0.6, label='ref', color='orange', edgecolor='black')

plt.xlabel('Ganancia')
plt.ylabel('Frecuencia')
plt.title(f' Exp vs control (p-value: <0.001')
...
plt.show()
 ...
 set(remaning_originals) -set(origianal_selection)
...
def terminate_vm(project_id, zone, instance_name):
    instances_client = compute_v1.InstancesClient()
    operation = instances_client.delete(project=project_id, zone=zone, instance=instance_name)
    print(f"Termination in progress for {instance_name}...")
...
# Example usage
project_id = "reflected-wind-441920-p5"
zone = "us-central1-a "  # e.g., "us-central1-a"
instance_name = "temp-08vcpu-032ram-20241120-043554"

terminate_vm(project_id, zone, instance_name)
...
###########################################  borradores
best_trial = study.trials[17]
def best_trial_objective(best_trial):
    
        
    params['learning_rate'] = best_trial.suggest_float("learning_rate", 0.3, 0.7)   
    params['feature_fraction'] = best_trial.suggest_float("feature_fraction", 0.3, 0.9)
    params['num_leaves'] = best_trial.suggest_int("num_leaves", 8, 2048)
    params['num_iterations'] = best_trial.suggest_int("num_iterations", 5, 50)
    min_data_in_leaf = best_trial.suggest_float("min_data_in_leaf",  1.5E-05, 0.009)  # Example of leaf size    
    
    params['neg_bagging_fraction'] = 0.05#best_trial.suggest_float("neg_bagging_fraction", 0.02, 0.35)  # Example of leaf size    
    params['pos_bagging_fraction'] = 1
    params['bagging_fraction'] = best_trial.suggest_float("bagging_fraction", 0.1, 1)
    params['max_bin']  = best_trial.suggest_int("max_bin",  31 , 255)  
  
    
    clase_peso_lgbm = best_trial.suggest_int('clase_peso_lgbm',2, ganancia_acierto+10000)   
    cant_semillas_ensamble = best_trial.suggest_int('cant_semillas_ensamble',20, 100)   
#    fraction = 0.1# best_trial.suggest_float('fraction', 0.01, 1)             
    #fraction  = best_trial.suggest_float('fracccion', 0.01, 0.2)   
    cantidad_meses = 3#best_trial.suggest_int('cantidad_meses', 6, 12)   
    best_trial_number = best_trial.number
    params['data_random_seed'] = best_trial_number
     
    
    random_state= best_trial_number
    ...
    for rnd in random_numbers:
        ...
        print('best_trial', best_trial_number, ' ensamble n: ', random_numbers.index(rnd))
        params['bagging_seed'] = rnd
        params['feature_fraction_seed'] = rnd+1
        params['seed'] =   rnd+2
        wstart= time.time()  
        ...
        mini_start= time.time()  
        y_test_pred =  exectue_model(  train_data, X_test , params, best_trial_number, min_data_in_leaf ,exp_folder,rnd)
        times.append(  time.time() - mini_start )
    ...
    for n_envios in [10000, 11000, 12000, 13000]:
        submission_path = to_kaggle_file (n_envios, res_mean, exp_folder , X_test,best_trial_number) 
...
def to_kaggle_file (n_envios, y_future, exp_folder , X_future, trial_number) :
    ...
    w_final_df = X_future[['numero_de_cliente']].copy()  # Use .copy() to avoid SettingWithCopyWarning
    w_final_df['Predicted'] = w_final  # Add the predictions
    submission_path = exp_folder + str(n_envios)+ 'trial_' + str(trial_number)+'.csv'
