...
def optimize_and_predict(X_train,X_train_final,X_future, X_test, y_train,y_train_final, y_test, target_col , n_trials=10):
    """
    Optimize model for a single target column and return predictions
    """
    study = optuna.create_study(direction='minimize')
    study.optimize(
        lambda trial: objective(trial, X_train, y_train[target_col], 
    ...
    future_pred = final_model.predict(X_future.fillna(X_test.mean()))
...
def forecast_numeric(data, features, trains, trains_final, mes_test, mes_future,  subsample_ratio=0.02, variance_threshold=1E2):
    ...
    df_train = data.filter(pl.col('foto_mes').is_in(trains))
    df_test = data.filter(pl.col('foto_mes') == mes_test)
    df_final_train = data.filter(pl.col('foto_mes') .is_in(trains_final))
    df_future = data.filter(pl.col('foto_mes') == mes_future)
    ...
    X_train_final = df_final_train.drop(['clase_ternaria', 'foto_mes', 'numero_de_cliente']).to_pandas()
    X_test = df_test.drop(['clase_ternaria', 'foto_mes', 'numero_de_cliente']).to_pandas()
    X_future = df_future.drop(['clase_ternaria', 'foto_mes', 'numero_de_cliente']).to_pandas()
    
    #X_train = X_train[feature_cols_X]
    #X_train_final = X_train_final[feature_cols_X]
    #X_test = X_test[feature_cols_X]
    #X_future = X_future[feature_cols_X]
    
    # Prepare target variables
    y_train = df_train[features].to_pandas()
    y_train_final = df_final_train[features].to_pandas()
    y_test = df_test[features].to_pandas()
    
    # Clean data
    y_train = y_train.dropna()
    y_test = y_test.dropna()
    y_train_final = y_train_final.dropna()
    X_train = X_train.loc[y_train.index]
    X_train_final = X_train_final.loc[y_train_final.index]
    X_test = X_test.loc[y_test.index]
    
    X_train = X_train.fillna(X_train_final.mean() ) 
    X_train_final = X_train_final.fillna(X_train_final.mean() )
    X_test = X_test.fillna(X_test.mean() )
                  
    
    
    # Filter high variance features
    variances = y_train.var()
    high_var = variances[variances > variance_threshold].index
    ...
    for col in high_var:
        ...
        future_pred, best_params,best_value = optimize_and_predict(X_train,X_train_final,X_future, X_test, y_train,y_train_final, y_test, col , 10)
    ...
    df_future_updated = pl.concat([df_future, future_predictions_pl], how='horizontal')
   
    #df_future_uptdated = pl.concat( [df_future, future_predictions_df])
    """
    # Add predictions to original data
    for col in high_var:
        pred_col_name = f"{col}_pred"
        df_train = df_train.with_columns(pl.Series(name=pred_col_name, values=train_predictions_df[col]))
        df_test = df_test.with_columns(pl.Series(name=pred_col_name, values=test_predictions_df[col]))
    """
    return df_future_updated #df_train, df_test, results['best_params']
...
def add_forecast_elasticnet( data,features_above_canritos):
    ...
    for w_fut_test in all_times[:-stride-window] :
        ...
        i_train_end = i_w_fut_test - stride
        i_train_start = i_train_end - window
        
        trains= all_times[i_train_start: i_train_end]
        test= all_times[i_train_end+2]
        final_train= all_times[ i_w_fut_test:i_w_fut_test+window ] 
        res[w_fut_test][ 'trains'] = trains
        res[w_fut_test][ 'test'] = test
        res[w_fut_test][ 'final_train'] = final_train
        print (res[w_fut_test])
        
    data_fut2 =[]    
    for mes_future in res.keys():    
        trains= res[mes_future][ 'trains']
        mes_test= res[mes_future][ 'test']
        trains_final= res[mes_future][ 'final_train']
    
        #mes_test= 202104
        #trains_final= [ 202103,202104 ]
        #future_pred= 202106
       
        df_future_updated = forecast_numeric(data, features_above_canritos, trains,trains_final, mes_test, mes_future,subsample_ratio, variance_threshold)
...
N_top, N_least,  mes_train, test_date, test_future = 15, 20, 202104, 202106, 202108
N_least_ampliado = 30
N_bins=5
