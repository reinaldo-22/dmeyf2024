...
div_sum_top_features <- function(data, top_features) {
  ...
  for (i in cols) {
    for (k in cols) {
      if (i != k) {
        # Get columns as float vectors for better performance
        col_i <- float::float(data_top[[i]])
        col_k <- float::float(data_top[[k]])
        
        # Calculate features
        new_cols[[index]] <- col_i * COS_PI4 - SIN_PI4 * col_k
        col_names[index] <- paste0(i, "_rot45c1_", k)
        index <- index + 1
        
        new_cols[[index]] <- col_i * SIN_PI4 + COS_PI4 * col_k
        col_names[index] <- paste0(i, "_rot45c2_", k)
        index <- index + 1
        
        new_cols[[index]] <- col_i - col_k
        ...
        new_cols[[index]] <- ifelse(col_k != 0, col_i / col_k, 0)
