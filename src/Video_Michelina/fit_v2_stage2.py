...
def lgb_gan_eval_LGBM_Early(y_pred, y_true ):
    ...
    return max_ganancia
def lgb_gan_eval_LGBM_Early(y_pred, dataset):
    ganancia_acierto = 273000
    costo_estimulo = 7000
    ...
    # Sort predictions and align profits accordingly
    sorted_indices = np.argsort(y_pred)[::-1]
    ganancia = ganancia[sorted_indices]
    
    # Calculate cumulative profit and find maximum
    ganancia = np.cumsum(ganancia)
    max_ganancia = np.max(ganancia)
    max_index = np.argmax(ganancia) + 1
    ...
    # Return the metric name, value, and a boolean indicating higher is better
    return 'max_ganancia', max_ganancia, True
...
def create_LGBM_dataset_th(final_selection, trains, mes_test, data_x, clase_peso_lgbm,params, mode_stacking, mode_FM, th_data, neg_bagging_fraction):
    ...
    print('Ratio >th +BAJAS / total', len(wd)/len(data_x))
    ...
    print('Ratio >th +CONTINUA / total', len(chosen_rows)/len(data_x))
...
def data_classification(data, suggest_params,   seed):   
    ganancia_acierto = 273000
    costo_estimulo =7000
    params = {
    ...
    if suggest_params != None:
        ...
        #Trial 8 '1st_study_250.joblib'
        default ={'learning_rate': 0.5166273878148264, 'feature_fraction': 0.46589209872112836, 'num_leaves': 439, 'num_iterations': 4, 'min_data_in_leaf': 0.004451849818627153, 'neg_bagging_fraction': 0.0391291098311023, 'bagging_fraction': 0.3031975172076743}
        #no rank
        default ={'learning_rate': 0.03, 'feature_fraction': 0.5, 'num_leaves': 689, 'num_iterations': 2000, 'min_data_in_leaf': 0.0001731723667393188, 'neg_bagging_fraction': 0.02 }
        #rank:
        default ={'learning_rate': 0.03, 'feature_fraction': 0.5, 'num_leaves': 512, 'num_iterations': 2000, 'min_data_in_leaf': 0.00015458917756666936, 'neg_bagging_fraction': 0.02 }
    ...
    if suggest_params == None:
        cant_semillas_ensamble = 50 # 250 cambio a 1000 para produccion
        seed=22
    else:
        cant_semillas_ensamble = 35
    cant_exp = 1
    ...
    final_train= list( data['foto_mes'].unique() )
    
    for mes_future in final_train[-9:-2]:
        if suggest_params != None:
            if mes_future!= final_train[-4]:
                continue
        i_mes = final_train.index(mes_future)
        trains= final_train[: i_mes-1]
        print('in data_classification', trains )
        print('in data_classification', mes_future )        
           
        params['data_random_seed'] =  seed          
        params['seed'] =  seed      
        mes_test = mes_future
        #data_x = pl.from_pandas(data)
        X_train, y_train,  w_train, X_test, y_test, params = create_LGBM_dataset_basic2 (final_selection, trains, mes_test, data, clase_peso_lgbm,params, neg_bagging_fraction)
        #th_data= 0.901
        #mode_stacking= True
        #mode_FM = True
        ...
        custom_lgbm2 = CustomLightGBM(params, cant_semillas_ensamble, mode, seed, min_data_in_leaf)
        custom_lgbm2.fit(X_train.to_numpy(), y_train.to_numpy().reshape(-1), X_test.to_numpy(), y_test.to_numpy().reshape(-1), )
        y_pred_fut_avg2= custom_lgbm2.predict_2(X_test.to_numpy())
        #solo para envios
        ...
        if type(y_test)!= type(None):
            ...
            futures[mes_future] =y_pred_fut_avg
            mask= y_pred_fut_avg>=th           
            clientes[mes_future]  =  set(X_test[mask]['numero_de_cliente'])
            
            print(  lgb_gan_eval_envios(y_pred_fut_avg.ravel(), y_test.values.ravel(), ganancia_acierto, costo_estimulo))                             
            
            
         ################   
        gancia_ensamble= {}
        for i in range(1, len(y_pred_fut_avg2[:,35])):
            gancia_ensamble[i] =  lgb_gan_eval_envios( np.mean( y_pred_fut_avg2[:i],axis=0).ravel(), y_test.values.ravel(), ganancia_acierto, costo_estimulo)[0]            
        n_ensambles = list(gancia_ensamble.keys())
        ganancia = list(gancia_ensamble.values())        
        plt.figure(figsize=(10, 6))
        plt.scatter(n_ensambles, ganancia, marker='o', linestyle='-', color='blue', label="Ganancia")        
        # Add labels to points
       # for i, g in enumerate(ganancia):
       #     plt.text(n_ensambles[i], g, f"{g}", fontsize=9, ha='center', va='bottom')        
        # Add titles and labels
        plt.title("Mes 202106 Ganancia vs N Ensambles")
        plt.xlabel("N Ensambles")
        plt.ylabel("Ganancia")
        plt.grid(True)
        plt.legend()
        plt.show()
         ...
         ##############
        meses = list(clientes.keys())
        envios={}
        for i in range(1, len(meses)):
            enviados=set()
            for iner in range(0,i):
                enviados = enviados.union(set( clientes[meses[iner]] ))
            
            current = clientes[meses[i]] 
            envios[ meses[i]]= len( current.difference(enviados ) )
            
            ##############
        mes_future = list(performance.keys())
        ganancia, envios, th = zip(*performance.values())                
        plt.figure(figsize=(10, 6))
        plt.scatter(envios, th, color="blue", s=100, label="Data Points")
        for i, mes in enumerate(mes_future):
            plt.text(envios[i], th[i], mes, fontsize=15, ha='right', va='bottom')        

        plt.title("Envios vs Threshold (th) por Mes_Future")
        plt.xlabel("Envios")
        ...
        mes_future = list(performance.keys())
        ganancia = [values[0] for values in performance.values()]
        
        # Convert integer months to strings for display
        mes_future_str = [str(mes) for mes in mes_future]
        
        # Plot ganancia vs mes_future
        plt.figure(figsize=(10, 6))
        plt.scatter(mes_future_str, ganancia, color="blue", s=100, label="Ganancia")
        
        # Add labels to points
        for i, mes in enumerate(mes_future_str):
            plt.text(mes, ganancia[i], f"{ganancia[i]}", fontsize=9, ha='center', va='bottom')
        
        # Add titles and labels
        plt.title("Ganancia vs Mes_Future")
        plt.xlabel("Mes_Future")
        plt.ylabel("Ganancia")
        plt.grid(True)
        plt.legend()
        plt.show()
        ...
        #####
        mes_future = list(performance.keys())
        ganancia, _, th = zip(*performance.values())
        ganancia, th, mes_future = ganancia[2:], th[2:], mes_future[2:]
        ...
        plt.title("Envios Nuevos vs Threshold (th) por Mes_Future")
        plt.xlabel("Envios")
        plt.ylabel("Threshold (th)")
        plt.grid(True)
        plt.show()
    ...
    data_x_pd.to_parquet(  desktop_folder+ 'data_x_final_fut2.parquet')
    data_x= pl.from_pandas(data_x_pd) 
    del(data_x_pd )    
    data_x_b =  data_x
    data_x = add_lags_future_predictions(data_x, 3)
    
    data_x.write_parquet(  desktop_folder+ 'data_x_final_fut2_lags.parquet')
...
class CustomLightGBM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y, X_test, y_test):
        ...
        val_data = lgb.Dataset(X_test, label=y_test, weight=[1] * len(y_test))
        ...
        for i in range(self.n_models):
            ...
            # Copy parameters for the current run
            ...
            model = lgb.train(
                w_params_run,
                train_data,
                valid_sets=[train_data, val_data],  # Specify both training and validation datasets
                valid_names=['train', 'val'],
                num_boost_round=2048,  # Maximum number of boosting rounds
                feval=lgb_gan_eval_LGBM_Early,
                callbacks=[ lgb.early_stopping(stopping_rounds=200, min_delta= 1E6, first_metric_only=True),  # Add early stopping callback
                    lgb.log_evaluation(5)  # Log evaluation every 10 rounds
                ])
...
def modify_params(params):
    ...
    param_keys = [ 'num_leaves', 'min_data_in_leaf']
...
def objective_data_classification(trial,data_w):
    ...
    params= {}
    params['learning_rate'] = 0.03 #trial.suggest_float("learning_rate", 0.25, 1)   
    params['feature_fraction'] = 0.5#trial.suggest_float("feature_fraction", 0.05, 0.9)
    params['num_leaves'] = trial.suggest_int("num_leaves", 500, 1524)
    params['num_iterations'] = 200# trial.suggest_int("num_iterations", 1, 50)
    params['min_data_in_leaf'] = trial.suggest_float("min_data_in_leaf",  1E-05, 2E-04)  # Example of leaf size        
    params['neg_bagging_fraction'] = 0.02  #trial.suggest_float("neg_bagging_fraction", 0.02, 0.05)  
...
def optuna_objective_data_classification():
    ...
    exp_folder = '/home/reinaldo_medina_gcp/buckets/b2/Michelina/'
    nombre_exp_study= 'intrames_lagas_frcts_timeFeatures.joblib'
    nombre_exp_study= 'intrames_lagas_frcts_timeFeatures_rank.joblib'
    nombre_exp_study= 'intrames_y_lagas.joblib'
    ...
    if os.path.exists(exp_folder+nombre_exp_study):
        ...
        data_x= pl.read_parquet( path_data_x)        
        #data_x = add_lags_future_predictions(data_x, 3)
       # data_w= data_x
    ...
    for i in range(0, 4):
        ...
        study.optimize(lambda trial: objective_data_classification(trial, data), n_trials=1, n_jobs=-1)
