...
envg$EXPENV$exp_dir <- "~/buckets/b1/expw227_c3_v0/"
envg$EXPENV$wf_dir <- "~/buckets/b1/flow227_c3_v0/"
envg$EXPENV$repo_dir <- "~/dmeyf2024/"
envg$EXPENV$datasets_dir <- "~/buckets/b1/datasets/"
envg$EXPENV$messenger <- "~/install/zulip_enviar.sh"

envg$EXPENV$semilla_primigenia <- 1021911
...
HT_tuning_semillerio <- function( pinputexps, semillerio, bo_iteraciones, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    max_bin = 41L, # lo debo dejar fijo, no participa de la BO
...
wf_SEMI_ago_orden227 <- function( pnombrewf )
{
  ...
  ht <- HT_tuning_semillerio(
    ...
    bo_iteraciones = 35  # iteraciones inteligentes, apenas 10
  )
